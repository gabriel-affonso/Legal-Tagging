...
def _apply_category_rules(result: ExtractionResult, text: str, reasons: list[str]) -> None:
    if result.document_category == "property_document":
        if result.lessor or result.lessee:
            if not re.search(
                r"\b(?:senhorio|arrendador|arrendat[aá]rio|inquilino|locador|locat[aá]rio)\b",
                text,
                re.IGNORECASE,
            ):
                result.lessor = ""
...
def _apply_review_rules(
    ...
    # Medium confidence alone is not sufficient reason for manual review.
    # Missing/blank or low confidence still requires review.
    ...
    if (
        signals.suggested_category != "other"
        and result.document_category != signals.suggested_category
    ):
        reasons.append(
            "category_changed_deterministic="
            f"{signals.suggested_category}_llm={result.document_category}"
        )
    ...
    if result.document_category == "lease_contract":
        _review_lease_contract(result, reasons)
    elif result.document_category == "property_document":
        _review_property_document(result, reasons)
    elif result.document_category == "bank_details":
        _review_bank_details(result, reasons)
    elif result.document_category != "payment_proof":
        # payment_proof is already checked by _apply_category_rules.
        _review_using_configured_fields(result, reasons)

    # extraction_notes remains available in the register for context, but does
    # not trigger review by itself.

    if result.iban and any(
        is_bank_like_value(getattr(result, field))
        for field in ["payment_amount", "property_name", "property_number"]
    ):
        reasons.append("iban_detected_in_suspicious_field")


def _review_lease_contract(result: ExtractionResult, reasons: list[str]) -> None:
    required_fields = ["lessor", "lessee", "signed_date", "monthly_rent"]
    for field_name in required_fields:
        if not getattr(result, field_name).strip():
            reasons.append(f"missing_{field_name}")

    # A contract has sufficient property identification when at least one
    # meaningful property descriptor or identifier is available.
    property_values = [
        result.property_display_name,
        result.property_name,
        result.property_number,
        result.property_article,
        result.property_address,
        result.property_location,
    ]
    if not any(value.strip() for value in property_values):
        reasons.append("missing_property_identification")


def _review_property_document(result: ExtractionResult, reasons: list[str]) -> None:
    # The mandatory set below applies specifically to a Caderneta Predial.
    # Other property-document subtypes may legitimately have different fields.
    if not _is_caderneta_predial(result):
        _review_using_configured_fields(result, reasons)
        return
...
def _review_bank_details(result: ExtractionResult, reasons: list[str]) -> None:
    for field_name in ["bank_account_holder", "bank_name"]:
        if not getattr(result, field_name).strip():
            reasons.append(f"missing_{field_name}")

    # IBAN and NIB are alternatives: one validly extracted identifier suffices.
    if not result.iban.strip() and not result.nib.strip():
        reasons.append("missing_iban_or_nib")


def _review_using_configured_fields(
    result: ExtractionResult,
    reasons: list[str],
) -> None:
...
def _is_caderneta_predial(result: ExtractionResult) -> bool:
    label = " ".join(
        [result.document_type, result.document_subtype]
    ).strip().lower()
    normalized = _strip_accents(label)
    return "caderneta predial" in normalized
