...
def _category_from_file_name(file_name: str, reasons: list[str]) -> str:
    ...
    lease_name_match = (
        ...
        or (contractual_action_match is not None and lease_code_match is not None)
    )
    ...
    checks = [
        ...
        (
            "bank_details",
            "IBAN/NIB/BIC/SWIFT",
            any(token in lowered for token in ["iban", "nib", "bic", "swift"]),
        ),
        (
            "lease_contract",
        ...
        (
            "payment_proof",
            "comprovativo/transferencia/pagamento",
            any(
                token in lowered
                for token in [
                    "comprovativo",
